...
import { convertProductImageToWebp } from "@/services/productImageUpload";
...
export function ProductImageUploadField({ productName, imagePath, onImagePathChange, prepareImage = convertProductImageToWebp, uploadImage }: {
  productName: string;
  imagePath: string;
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setError(null);
    try {
      const webp = await prepareImage(file);
      const stored = await uploadImage(webp);
      onImagePathChange(stored.path);
    } catch (uploadError) {
      setError(uploadError instanceof Error ? uploadError.message : "Não foi possível preparar a foto do produto.");
    } finally {
      setUploading(false);
      event.target.value = "";
    }
  };
  return <div className="mt-4 space-y-2"><Label htmlFor="product-image-upload">Foto do produto</Label><Input id="product-image-upload" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void handleFileChange(event)} disabled={uploading} /><p className="text-xs text-[#765f50]">Envie JPG, PNG ou WebP de até 5 MB. A imagem será convertida para WebP antes do armazenamento.</p>{uploading && <p role="status" className="text-sm text-[#68703d]">Convertendo e enviando foto…</p>}{error && <p role="alert" className="text-sm text-[#a82926]">{error}</p>}<ProductImagePreview productName={productName} imagePath={imagePath} /></div>;
}
