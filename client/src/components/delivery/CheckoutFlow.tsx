...
import { ArrowLeft, ArrowRight, Banknote, Bike, Check, CreditCard, MapPin, Package, Ticket } from "lucide-react";
import { useForm } from "react-hook-form";
import type { CheckoutDraft, OrderConfirmation, PaymentMethod } from "@shared/order";
import { useOrder } from "@/contexts/OrderContext";
import { formatCurrency } from "@/lib/order";
import { localOrderService } from "@/services/orderService";
...
export function CheckoutFlow({ onSuccess, onBack }: CheckoutFlowProps) {
  ...
  async function submitOrder() {
    ...
    const confirmation = await localOrderService.submit({
      id: typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `local-${Date.now()}`,
      createdAt: new Date().toISOString(),
      deliveryMode,
      customer: { ...customer, paymentMethod: selectedPayment as PaymentMethod },
      items,
      summary,
    });
    setIsSubmitting(false);
    onSuccess(confirmation, { items, summary, deliveryMode });
  }
