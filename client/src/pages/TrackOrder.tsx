...
import { ArrowLeft, CircleCheck, Clock3, CreditCard, MapPin, PackageCheck, Search, ShieldCheck } from "lucide-react";
...
export default function TrackOrder() {
  ...
  const [submitted, setSubmitted] = useState<{ code: string; phone: string } | null>(null);
  const tracking = trpc.orders.track.useQuery(
    { code: submitted?.code ?? "_", phone: submitted?.phone ?? "_" },
    { enabled: Boolean(submitted), retry: false },
  );

  const order = tracking.data?.order;
  const timeline = tracking.data?.events ?? [];
  ...
  return (
    <main className="min-h-screen bg-[#fffaf1] text-[#481e1f]">
      ...
      <section className="mx-auto grid max-w-5xl gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[0.85fr_1.15fr] lg:py-16">
        <div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#a82926]">Acompanhamento</p><h1 className="font-display mt-2 text-4xl tracking-[-0.04em] sm:text-5xl">Seu pedido, em cada etapa.</h1><p className="mt-4 max-w-md text-sm leading-relaxed text-[#765f50]">Informe o código do pedido e o telefone usado no checkout. Assim, somente você acompanha o preparo e a entrega.</p><div className="mt-7 rounded-3xl bg-[#f3ead8] p-5 text-xs leading-relaxed text-[#765f50]"><ShieldCheck className="mb-3 size-5 text-[#68703d]" /><strong className="block text-[#481e1f]">Consulta protegida</strong> O código e o telefone precisam coincidir com os dados do pedido para exibir as informações.</div></div>

        <div className="rounded-[2rem] border border-[#ead8c0] bg-white p-5 shadow-[0_16px_40px_rgba(72,30,31,0.08)] sm:p-7">
          <form onSubmit={(event) => { event.preventDefault(); setSubmitted({ code: code.trim().toUpperCase(), phone: phone.trim() }); }} className="grid gap-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
            <div><Label htmlFor="track-code" className="font-bold">Código do pedido</Label><Input id="track-code" value={code} onChange={(event) => setCode(event.target.value)} placeholder="TB-20260817-0001" className="mt-2 h-11 border-[#ead8c0]" /></div>
            ...
            <Button type="submit" disabled={tracking.isFetching || !code.trim() || !phone.trim()} className="h-11 rounded-xl bg-[#a82926] px-5 font-extrabold hover:bg-[#8e1718]"><Search className="mr-2 size-4" /> Consultar</Button>
          </form>

          {tracking.isFetching && <div className="mt-8 flex items-center gap-3 rounded-2xl bg-[#fff7eb] p-4 text-sm font-semibold text-[#765f50]"><Clock3 className="size-5 animate-spin text-[#a82926]" /> Consultando o status do pedido...</div>}
          {submitted && !tracking.isFetching && !order && <div className="mt-8 rounded-2xl bg-[#fff1ed] p-4 text-sm leading-relaxed text-[#8e3025]"><strong>Não encontramos esse pedido.</strong><br />Confira o código e use o mesmo telefone informado no checkout.</div>}
          ...
          {order && <div className="mt-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-[#a58f7a]">Pedido {order.code}</p><h2 className="font-display mt-1 text-3xl tracking-[-0.04em]">{statusLabels[order.status]}</h2></div><span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${order.paymentStatus === "confirmed" ? "bg-[#e6efd0] text-[#53652b]" : "bg-[#f7e7d5] text-[#8d3c2f]"}`}>{order.paymentStatus === "confirmed" ? "Pagamento confirmado" : order.paymentMethod === "cash" ? "Pagamento no recebimento" : "Pagamento pendente"}</span></div><div className="mt-4"><PaymentModeNotice mode={toPaymentNoticeModeFromProvider(order.paymentProvider)} /></div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#f8f0e3] p-4"><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[#8b735f]"><PackageCheck className="size-4 text-[#a82926]" /> Recebimento</p><p className="mt-2 text-sm font-bold text-[#481e1f]">{order.fulfillmentMethod === "delivery" ? "Entrega no endereço informado" : "Retirada na Marmitas TB"}</p>{order.fulfillmentMethod === "delivery" && order.deliveryAddress && <p className="mt-1 text-xs leading-relaxed text-[#765f50]">{order.deliveryAddress}</p>}</div><div className="rounded-2xl bg-[#f8f0e3] p-4"><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[#8b735f]"><CreditCard className="size-4 text-[#a82926]" /> Total</p><p className="mt-2 text-xl font-black text-[#a82926]">{formatCurrency(order.totalInCents / 100)}</p><p className="mt-1 text-xs text-[#765f50]">{order.paymentProvider === "asaas_test" ? "Pagamento em modo de teste" : "Pagamento processado pelo Asaas"}</p></div></div>
