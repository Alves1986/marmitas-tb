...
O log de runtime desse deployment confirmou a causa do `FUNCTION_INVOCATION_FAILED`: `ERR_MODULE_NOT_FOUND: Cannot find module '/var/task/server/vercel/_lib/http' imported from /var/task/api/public/menu.js`. Portanto, a falha não é de autenticação ou de dados do Supabase. As funções em `api/` foram implantadas sem o pacote local compartilhado sob `server/vercel/_lib`. Nenhuma alteração foi feita no Supabase durante o diagnóstico.

A primeira tentativa de corrigir o empacotamento, no commit `a147173`, gerou o deployment `J91yLpSFHYDAyg8AfB8d6ft6mGgm`, que falhou antes de iniciar porque a Vercel recusou o formato de lista em `functions.api/**/*.ts.includeFiles`: o esquema exige uma **string**. O diagnóstico foi obtido no painel autenticado. A correção subsequente preserva os módulos compilados e os nove handlers, mas usa o padrão de string com chaves aceito pela Vercel: `{server/vercel/_lib/**/*.js,shared/operations.js}`. A proteção estrutural e o build local foram atualizados e validados antes da nova implantação.
...
O teste OTP autorizado para `cassia.andinho@gmail.com` confirmou que não havia usuário em `auth.users`; o Supabase recusou corretamente o envio com `Signups not allowed for otp`, sem criar cadastro automático. Após autorização explícita para o papel `admin`, foi enviado um único convite pelo painel Supabase. O usuário recebeu o UUID `58e6ea60-e467-45b3-b5c7-176163de5275`; o perfil inicialmente criado como `customer` foi promovido para `admin` por uma atualização restrita a esse UUID, cuja confirmação retornou exclusivamente o mesmo identificador e o papel `admin`. Nenhum outro usuário, papel ou configuração de criação automática foi modificado.

### Dependência de SMTP para o OTP por código

Com a conta administrativa já provisionada, a solicitação de OTP foi aceita pelo Supabase e a tela de acesso da aplicação entrou na etapa de código de seis dígitos. A pessoa responsável recebeu dois e-mails, mas nenhum continha o código esperado. A verificação do código-fonte confirmou que a aplicação chama `signInWithOtp` com `shouldCreateUser: false` e valida exclusivamente `verifyOtp({ type: "email", token })`; portanto, o contrato do produto é **código numérico**, não magic link.

No painel de templates, o Supabase informou que os envios estão usando os modelos padrão e que a edição do assunto e do corpo exige SMTP customizado. O formato atual enviou o convite e/ou link de acesso padrão, incompatível com o formulário que espera o token. Por decisão explícita, a aplicação **não** será adaptada para magic link nesta fase e o cadastro automático permanece bloqueado. A validação final do OTP foi adiada até a configuração de um provedor SMTP transacional, cujo template deve incluir o token de seis dígitos. Nenhum novo envio de e-mail, alteração de template ou relaxamento de segurança foi realizado após essa decisão.

Na conferência posterior do painel Vercel, os quatro deployments mais recentes da branch `feat/supabase-vercel-migration` estavam em estado **Ready** e classificados como **Preview** (`878fe83`, `106aaf6`, `672f59f` e `9ff17d2`). O único deployment classificado como Production é o incidente histórico `4f76d4b`, em estado **Error**, sem servir a aplicação nem representar uma promoção posterior. Nenhuma nova implantação de produção foi acionada durante a homologação.
