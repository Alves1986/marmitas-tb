...
import { createOperationsResourceHandler, operationResources } from "../api/operations/[resource]";

describe("dispatcher de recursos operacionais", () => {
  it.each(["orders", "alerts", "inventory", "printJobs", "help"])("preserva a URL operacional de %s", async (resource) => {
    const handler = createOperationsResourceHandler({
      ...
      help: () => async () => json(200, { resource: "help" }),
    });
  ...
  it("registra ajuda no conjunto de recursos consolidados", () => {
    expect(operationResources).toContain("help");
  });
